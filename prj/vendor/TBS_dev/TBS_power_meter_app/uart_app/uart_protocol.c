...
void uart_driver_get_stats(
    ...
    if (total_rx_bytes) *total_rx_bytes = driver->total_rx_bytes;
    if (total_tx_bytes) *total_tx_bytes = driver->total_tx_bytes;
    if (rx_overflow_count) *rx_overflow_count = driver->rx_overflow_count;
}
...
void protocol_get_stats(
    ...
    if (tx_success) *tx_success = protocol->tx_success_count;
    if (tx_fail) *tx_fail = protocol->tx_fail_count;
    if (rx_success) *rx_success = protocol->rx_success_count;
    if (rx_crc_error) *rx_crc_error = protocol->rx_crc_error_count;
    if (rx_frame_error) *rx_frame_error = protocol->rx_frame_error_count;
}
