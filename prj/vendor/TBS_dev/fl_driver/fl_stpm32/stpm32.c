...
float stpm_read_power_factor(stpm_handle_t *handle, uint8_t channel)
{
    ...
    read_frame(handle, address, handle->read_buffer);
    int32_t active = buffer_0to28(handle->read_buffer);

    address = (channel == 1) ? PH1_Apparent_RMS_Power_Address : PH2_Apparent_RMS_Power_Address;
    read_frame(handle, address, handle->read_buffer);
...
static void read_frame(stpm_handle_t *handle, uint8_t address, uint8_t *buffer) {
    uint8_t rx_frame[STPM3x_FRAME_LEN];
    uint8_t tx_frame[STPM3x_FRAME_LEN] = {address, 0xff, 0xff, 0xff};
    handle->spi_begin_transaction();
    handle->pin_write(handle->cs_pin, false);
    // handle->spi_transfer(address);
    // handle->spi_transfer(0xff);
    // handle->spi_transfer(0xff);
    // handle->spi_transfer(0xff);
    handle->spi_transfer_buffer(tx_frame, rx_frame, STPM3x_FRAME_LEN);
    handle->spi_transfer_buffer(tx_frame, buffer, STPM3x_FRAME_LEN);
    handle->pin_write(handle->cs_pin, true);
    handle->spi_end_transaction();
...
static void send_frame(stpm_handle_t *handle, uint8_t read_add, uint8_t write_add,
    ...
    handle->spi_begin_transaction();
    handle->pin_write(handle->cs_pin, false);
    handle->spi_transfer_buffer(frame, rx_frame, STPM3x_FRAME_LEN);
